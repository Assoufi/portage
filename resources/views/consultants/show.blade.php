...
@section('content')
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="font-medium">{{ $consultant->tel }}</p>
                        </div>
                        
                        <div>
                            <p class="text-sm text-gray-600">RIB</p>
                            <p class="font-medium">{{ $consultant->rib ?? 'Non renseigné' }}</p>
                        </div>
                        
                        <div>
                            <p class="text-sm text-gray-600">Mode de paiement</p>
                            <p class="font-medium">{{ $consultant->mode_paiement_label }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="font-medium">{{ $consultant->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                        
                        <div>
                            <p class="text-sm text-gray-600">Dernière modification</p>
                            <p class="font-medium">{{ $consultant->updated_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Dernières missions -->
    @if($consultant->missions->count() > 0)
        ...
        <div class="p-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody>
                        @foreach($consultant->missions as $mission)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-right">{{ number_format($mission->prix_vente, 2) }} {{ $mission->client->devise }}</td>
