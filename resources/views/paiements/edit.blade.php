                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="client_id" id="client_id"
                            ...
                            @foreach($clients as $client)
                                <option value="{{ $client->id }}" {{ old('client_id', $paiement->client_id) == $client->id ? 'selected' : '' }}>
                                    {{ $client->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="fournisseur_id" id="fournisseur_id"
                            ...
                            @foreach($fournisseurs as $fournisseur)
                                <option value="{{ $fournisseur->id }}" {{ old('fournisseur_id', $paiement->fournisseur_id) == $fournisseur->id ? 'selected' : '' }}>
                                    {{ $fournisseur->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="mission_id" id="mission_id"
                            ...
                            @foreach($missions as $mission)
                                <option value="{{ $mission->id }}" {{ old('mission_id', $paiement->mission_id) == $mission->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" name="montant_recu" id="montant_recu" step="0.01"
                               value="{{ old('montant_recu', $paiement->montant_recu) }}"
                    ...
                    <div>
                        ...
                        <select name="mode_paiement" id="mode_paiement"
                                ...
                                required>
                            <option value="virement" {{ old('mode_paiement', $paiement->mode_paiement?->value) == 'virement' ? 'selected' : '' }}>Virement bancaire</option>
                            <option value="cheque" {{ old('mode_paiement', $paiement->mode_paiement?->value) == 'cheque' ? 'selected' : '' }}>Chèque</option>
                            <option value="especes" {{ old('mode_paiement', $paiement->mode_paiement?->value) == 'especes' ? 'selected' : '' }}>Espèces</option>
                            <option value="carte" {{ old('mode_paiement', $paiement->mode_paiement?->value) == 'carte' ? 'selected' : '' }}>Carte bancaire</option>
                        </select>
                        @error('mode_paiement')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="statut" id="statut"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200">
                            <option value="1" {{ old('statut', $paiement->statut) == '1' ? 'selected' : '' }}>Actif</option>
                            <option value="0" {{ old('statut', $paiement->statut) == '0' ? 'selected' : '' }}>Inactif</option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="remarques" id="remarques" rows="4"
                                  class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 @error('remarques') border-red-500 @enderror">{{ old('remarques', $paiement->remarques) }}</textarea>
                        @error('remarques')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
